...
resource "nutanix_protection_rule" "rule" {
  for_each = var.protection_rules

  name        = each.value.name
  description = each.value.description

  dynamic "ordered_availability_zone_list" {
    for_each = each.value.ordered_availability_zone_list != null ? each.value.ordered_availability_zone_list : []
    content {
  ...
  dynamic "availability_zone_connectivity_list" {
    for_each = each.value.availability_zone_connectivity_list != null ? each.value.availability_zone_connectivity_list : []
    content {
      ...
      dynamic "snapshot_schedule_list" {
        for_each = availability_zone_connectivity_list.value.snapshot_schedule_list != null ? availability_zone_connectivity_list.value.snapshot_schedule_list : []
        content {
          ...
          dynamic "local_snapshot_retention_policy" {
            for_each = snapshot_schedule_list.value.local_snapshot_retention_policy != null ? [snapshot_schedule_list.value.local_snapshot_retention_policy] : []
            content {
          ...
          dynamic "remote_snapshot_retention_policy" {
            for_each = snapshot_schedule_list.value.remote_snapshot_retention_policy != null ? [snapshot_schedule_list.value.remote_snapshot_retention_policy] : []
            content {
  ...
  dynamic "category_filter" {
    for_each = each.value.category_filter != null ? [each.value.category_filter] : []
    content {
      type      = category_filter.value.type
      kind_list = category_filter.value.kind_list

      dynamic "params" {
        for_each = category_filter.value.params != null ? category_filter.value.params : []
        content {
